...
```
"""
function DensityX(oxd_wt::Vector{_T}, T::_T, P::_T) where {_T<:Number}
    ...
    norm_WP = oxd_wt ./ sum(oxd_wt) .* 100.0
    ...
    part_Vliq = (MV .+ (dVdT .* (T_K .- Tref)) .+ (dVdP .* (P_bar .- 1.0))) .* norm_MP
